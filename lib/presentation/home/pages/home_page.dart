...
import 'package:geo_presence/core/helper/radius_calculate.dart';
import 'package:geo_presence/presentation/home/bloc/get_company/get_company_bloc.dart';
import 'package:geo_presence/presentation/home/bloc/is_checkedin/is_checkedin_bloc.dart';
import 'package:geo_presence/presentation/home/pages/attendance_checkout_page.dart';
            ...
            children: [
              ...
              Padding(
                ...
                child: GridView(
                  ...
                  physics: const NeverScrollableScrollPhysics(),
                  children: [
                    BlocListener<IsCheckedinBloc, IsCheckedinState>(
                      ...
                      child: BlocBuilder<GetCompanyBloc, GetCompanyState>(
                        builder: (context, state) {
                          final latPoint = state.maybeWhen(
                            orElse: () => 0.0,
                            success: (data) =>
                                double.tryParse(data.latitude!) ?? 0.0,
                          );

                          final longPoint = state.maybeWhen(
                            orElse: () => 0.0,
                            success: (data) =>
                                double.tryParse(data.longitude!) ?? 0.0,
                          );

                          final radiusKmPoint = state.maybeWhen(
                            orElse: () => 0.0,
                            success: (data) =>
                          ...
                          final distanceKm = RadiusCalculate.calculateDistance(
                              latitude!, longitude!, latPoint, longPoint);
                          return MenuButton(
                            label: 'Datang',
                            ...
                            onPressed: () async {
                              ...
                              if (isFakeLocation) {
                              ...
                              } else if (distanceKm > radiusKmPoint) {
                                if (context.mounted) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content:
                                          Text('Anda diluar jangkuan Absen'),
                                      backgroundColor: AppColors.red,
                                    ),
                                  );
                                }
                                ...
                                if (context.mounted) {
                                  context.read<IsCheckedinBloc>().add(
                                      const IsCheckedinEvent.isCheckedIn());
                                }
                              }
                            },
                          );
                    ...
                    MenuButton(
                      ...
                      onPressed: () => context.push(const PermissionPage()),
                    ),
                    MenuButton(
                      label: 'Catatan',
